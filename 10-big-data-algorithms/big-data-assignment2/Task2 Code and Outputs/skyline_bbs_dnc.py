...
def merge_skylines(left, right):
    ...
    combined = left + right
    for p in combined:
        if not any(dominates(q, p) for q in combined if q != p):
            final.append(p)
    return final
...
def write_output(skyline_points, output_path, runtime):
    with open(output_path, 'a') as f:
        f.write("=======================================\n")
        f.write("Algorithm: BBS with Divide and Conquer (by X)\n")
...
if __name__ == "__main__":
    ...
    # Step 1: Split by X (cost)
    dataset.sort(key=lambda x: x[1])  # sort by cost
    mid = len(dataset) // 2
    ...
    # Step 2: BBS on both halves
    left_skyline = bbs_simple(left_half)
    right_skyline = bbs_simple(right_half)

    # Step 3: Merge skylines
    final_skyline = merge_skylines(left_skyline, right_skyline)

    end_time = time.time()
    duration = end_time - start_time

    print(f"Skyline has {len(final_skyline)} points. Took {duration:.6f} seconds.")
    write_output(final_skyline, output_path, duration)
